...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for particle in beliefDist:
      _distance = util.manhattanDistance(pacmanPosition, particle)
      beliefDist[particle] *= emissionModel[_distance]
    beliefDist.normalize()
    if beliefDist.totalCount() == 0.0:
      self.initializeUniformly(gameState)
    else:
      self.particles = [util.sample(beliefDist) for index in range(self.numParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    for particle in self.particles:
      _distributions = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
      _particles.append(util.sample(_distributions))
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for particleIndex in range(self.numParticles):
      self.particles.append(tuple(combinations[particleIndex % len(combinations)]))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      for index in range(self.numGhosts):
        _posDist = getPositionDistributionForGhost(
          setGhostPositions(gameState, newParticle),
          index,
          self.ghostAgents[index]
        )
        newParticle[index] = util.sample(_posDist)
    ...
    for particle in self.particles:
      ...
      for ghostIndex in range(self.numGhosts):
        if noisyDistances[ghostIndex] != None:
          _distance = util.manhattanDistance(pacmanPosition, particle[ghostIndex])
          weightedParticles[particle] *= emissionModels[ghostIndex][_distance]
      ...
      for ghostIndex in range(self.numGhosts):
        if noisyDistances[ghostIndex] == None:
          for i in range(self.numParticles):
            ...
            _particle[ghostIndex] = self.getJailPosition(ghostIndex)
